...
protected:
    ...
    /**
     * @brief   绘制绘制对象（艺术网格）<br>
     *           传递一组多边形网格和纹理编号。
     *
     * @param[in]   textureNo            ->  要绘制的纹理编号
     * @param[in]   indexCount           ->  绘图对象的索引值
     * @param[in]   vertexCount          ->  多边形网格顶点数
     * @param[in]   indexArray           ->  多边形网格顶点索引阵列
     * @param[in]   vertexArray          ->  多边形网格顶点阵列
     * @param[in]   uvArray              ->  uv数组
     * @param[in]   opacity              ->  不透明度
     * @param[in]   colorBlendMode       ->  颜色混合类型
     * @param[in]   invertedMask          ->  使用遮罩时反转使用遮罩
     *
     */

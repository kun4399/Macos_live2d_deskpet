...
void CubismRenderer_OpenGLES2::DoDrawModel()
{
    ...
    for (csmInt32 i = 0; i < drawableCount; ++i)
    {
        ...
        if (clipContext != NULL && IsUsingHighPrecisionMask()) // マスクを書く必要がある？你需要写一个面具
        {
            if(clipContext->_isUsing) // 書くことになっていた？决定写了
            {
                // 将视口设置为与生成的帧缓冲区大小相同
                ...
                // 清除遮罩
                // 1无效（未绘制）区域，0有效（绘制）区域。（在着色器中，用Cd*Cs乘以接近0的值制作掩模。乘以1后什么都不会发生）
                glClearColor(1.0f, 1.0f, 1.0f, 1.0f);
                glClear(GL_COLOR_BUFFER_BIT);
            }

            {
                ...
                for (csmInt32 index = 0; index < clipDrawCount; index++)
                {
                    ...
                    DrawMeshOpenGL(
                        ...
                        CubismRenderer::CubismBlendMode_Normal,   //裁剪通常强制绘制
                        false // 生成遮罩时，与剪辑的反转使用完全没有关系
                    );
                }
            }
        ...
        DrawMeshOpenGL(
            ...
            GetModel()->GetDrawableInvertedMask(drawableIndex) // 使用翻转遮罩
        );
    }
